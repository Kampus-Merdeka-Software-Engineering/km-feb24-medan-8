...
function calculateTotalProfit(stateFiltersValues, countryFiltersValues, subCategoryValues, dateRangeFilterValues) {
  fetch("data.json")
...
function calculateTotalOrder(stateFiltersValues, countryFiltersValues, subCategoryValues, dateRangeFilterValues) {
  fetch("data.json")
    .then((response) => response.json())
    .then((data) => {
      let filteredData = processFilter(data,
